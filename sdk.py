...
class sdk:
    ...
    def get_hwio_signal_descriptor(self, signal_number):
        ...
        ret = {}
        if error == 0:
            ret.update({'hwio signal descriptor': 1})
    ...
    def get_hwio_signal(self, index):
        ...
        ret = {}
        if error == 0:
            wEnabled_states = {0x0000: 'Signal is off',
                               0x0001: 'Signal is active'}

            wType_states = {0x0001: 'TTL',
                            0x0002: 'High Level TTL',
                            0x0004: 'Contact Mode',
                            0x0008: 'RS485 differential',
                            0x0080: 'Two pin differential TTL PinA=TTL, PinB=GND'}
            ...
            wFilter_state = {0x0000: '----------------------error',
                             0x0001: 'No signal filtering (t > ~65 ns)',
                             0x0002: 'MediumFilter (t > ~1 μs)',
                             0x0004: 'High Filter (t > ~100ms)'}
    ...
    # -------------------------------------------------------------------------
    def set_hwio_signal(self, index, enabled, signal_type, polarity, filter_type):
        """
    ...
    def get_cmos_line_timing(self):
       ...
       """
        ...
        If this mode is active, timing related settings from the functions in TIMING CONTROL are inactive
        and current delay and exposure times must be readout with function
        PCO_GetCmosLineExposureDelay.
        The resulting exposure time in µs can be calculated as:
        int64 expos=dwExposureLines*dwLineTime*(1000ˆdwTimebase).
        ...
        if error == 0:
            ...
            ret.update({'time base': wTimeBase.value})
            ret.update({'line time': dwLineTime.value})
    ...
    # -------------------------------------------------------------------------
    def set_cmos_line_timing(self, parameter, timebase, linetime):

        """
        Set CMOS Line Timing

        This function enables or disables the Lightsheet scanning mode of the pco.edge. In Lightsheet
        scanning mode exposure and delay time setting is correlated to the sensor line time. If this mode
        is enabled, timing related settings from the functions in TIMING CONTROL can not be used.
        The setting of delay time and exposure time must be done with the function
        PCO_SetCmosLineExposureDelay.
        The resulting exposure-time can be calculated as:
        dwExposureLines*dwLineTime*(1000ˆdwTimebase)
        ...
        parameter_types = {'on': 0x0001, 'off': 0x0000}
        time_base = {'ns': 0x0000, 'us': 0x0001, 'ms': 0x0002}

        wParameter = C.c_uint16(parameter_types[parameter])
        wTimeBase = C.c_uint16(time_base[timebase])
        ...
        error = self.SC2_Cam.PCO_SetCmosLineTiming(self.camera_handle,
                                                                  ...
                                                                  wReservedLen)
        inp = {}
        inp.update({'parameter': wParameter,
                    'time base': wTimeBase,
                    'line time': dwLineTime.value})
    ...
    def get_cmos_line_exposure_delay(self):
        ...
        This function returns the current setting of the exposure and delay line settings for the Lightsheet
        scanning mode. It is only available when Lightsheet scanning mode has been successfully
        enabled by PCO_SetCmosLineTiming.
        """
        ...
        ret = {}
        if error == 0:
            ret.update({'exposure lines': dwExposureLines.value})
            ret.update({'delay lines': dwDelayLines.value})
    ...
    # -------------------------------------------------------------------------
    def set_cmos_line_exposure_delay(self, exposure_lines, delay_lines):

        """
        Set CMOS line exposure delay

        This function sets the exposure and delay line settings for the Lightsheet scanning mode. It is
        only available when Lightsheet scanning mode has been successfully enabled by
        ...
        dwExposureLines = C.c_uint32(exposure_lines)
        dwDelayLines = C.c_uint32(delay_lines)
        dwReserved = C.c_uint32()
        wReservedLen = C.c_uint16()
        ...
        error = self.SC2_Cam.PCO_SetCmosLineExposureDelay(self.camera_handle,
                                                                  ...
                                                                  wReservedLen)
        inp = {}
        inp.update({'exposure lines': dwExposureLines.value,
                    'delay lines': dwDelayLines.value})
